...
GET /api/alerts?scenario=  → 拉天气 + 当前物候期 + 硬规则评估 → 生效预警列表。
...
def register(app, ctx) -> None:
    ...
    @r.get("/api/alerts")
    async def alerts(scenario: str | None = None):
        wv = await weather.get(scenario)
        ...
        active = evaluate_alerts(wv, stage_key, ctx.knowledge_dir)
        return {
            "weather": {
                "source": wv.source,
