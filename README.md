...
- **检索接入问答**：chat 上下文新增"灌入资料"检索片段（标注出处），回答有据可依
- **种子资料**：`data/corpus/peach|apple/管理要点.md`（真实质量,待农技审核）
- 用户拿到真实《技术规程》《病虫情报》丢进对应文件夹 → 跑 ingest → AI 立刻更准
- 验证：问"萌芽前清园用啥药"→ AI 引用灌入资料作答（石硫合剂/缩叶病）；41 测试
- 后续：语料增大可换向量检索(chromadb)，retrieve 接口不变
